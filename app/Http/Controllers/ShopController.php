...
class ShopController extends Controller
{
    public function index(Request $request): Factory|View|Application
    {
        ...
        return view('shop.shop', compact('products', 'size', 'order', 'brands', 'f_brands', 'categories', 'f_categories', 'min_price', 'max_price'));
    }

    public function product_details($product_name): Factory|View|Application
    {
        $product = Product::where('name', $product_name)
            -> first();

        $rproducts = Product::where('name','<>', $product_name)
            -> get()
            -> take(8);
