                            ...
                            <div class="swiper-container">
                                <div class="swiper-wrapper">
                                    <div class="swiper-slide product-single__image-item">
                                        <img loading="lazy" class="h-auto" src="{{ asset('upload/categories') }}/{{ $category->image }}" width="674"
                                             height="674" alt="" />
                                        <a data-fancybox="gallery" href="{{ asset('upload/categories') }}/{{ $category->image }}" data-bs-toggle="tooltip"
                    ...
                    <div class="product-single__addtolinks">
                        @if(Cart::instance('wishlist')->content()->where('id', $product->id)->count() > 0)
                            <form method="POST" action="{{ route('wishlist.item.remove', ['rowId' => Cart::instance('wishlist')->content()->where('id', $product->id)->first()->rowId]) }}" id="frm-remove-item">
                                @csrf
                                @method("DELETE")
                                <a href="javascript:void(0)" class="menu-link menu-link_us-s add-to-wishlist filled-heart" onclick="document.getElementById('frm-remove-item').submit();" style="color: orange">
                        ...
                        <share-button class="share-button">
                            <button class="menu-link menu-link_us-s to-share border-0 bg-transparent d-flex">
                                <svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        @foreach($rproducts as $rproduct)
                            <div class="swiper-slide product-card">
                                <div class="pc__img-wrapper">
                                    <a href="{{ route('shop.product.details', ['product_name' => $rproduct->name]) }}">
                                        <img loading="lazy" src="{{ asset('upload/products') }}/{{ $rproduct->image }}" width="330" height="400"
                                             alt="{{ $rproduct->name }}" class="pc__img">
                                        @foreach(explode(',', $rproduct->images) as $rgimg)
                                            <img loading="lazy" src="{{ asset('upload/products') }}/{{ $rgimg }}" width="330" height="400"
                                    ...
                                    @else
                                        <form name="addtocart-form" method="post" action="{{ route('cart.add') }}">
                                            ...
                                            <input type="hidden" name="quantity" value="1">
                                            <input type="hidden" name="price" value="{{ $rproduct->sales_price == '' ? $rproduct->regular_price : $rproduct->sales_price }}">
                                            <button type="submit"
                                ...
                                <div class="pc__info position-relative">
                                    <p class="pc__category">{{ $rproduct->categories->name }}</p>
                                    <h6 class="pc__title"><a href="{{ route('shop.product.details', ['product_name' => $rproduct->name]) }}">{{ $rproduct->name }}</a></h6>
                                    <div class="product-card__price d-flex">
                                        <span class="money price">
                                            @if($rproduct->sales_price)
                                                <s>${{ $rproduct->regular_price }}</s> ${{ $rproduct->sales_price }}
                                            @else
                                                ${{ $rproduct->regular_pirce }}
                                            @endif
                                        </span>
                                    </div>
                                    @if(Cart::instance('wishlist')->content()->where('id', $product->id)->count() > 0)
